...
function renderRuns(){
 ...
 fetch('/api/runs',{cache:'no-store'}).then(x=>x.json()).then(runs=>{
  ...
  replayRuns=runs.filter(run=>run.status==='COMPLETED');$('replay-run').replaceChildren();$('analysis-run').replaceChildren();replayRuns.forEach(run=>{const option=document.createElement('option');option.value=run.run_id;const started=run.start_time?new Date(run.start_time).toLocaleString('pl-PL'):'—';option.textContent=`${started} · ${run.external_samples??0} GPS · ${run.run_id.slice(0,8)}`;$('replay-run').append(option);$('analysis-run').append(option.cloneNode(true));});
