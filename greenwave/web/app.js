...
function render() {
 ...
 if(phone)$('phone-values').textContent+=`\nŹródło lokalizacji: ${phone.source}${phone.device_id?' · '+phone.device_id.slice(0,8):''} · stan ruchu: ${connection?.motion_state??'—'} · transmisje: ${connection?.transmission_count??'—'} · duplikaty: ${connection?.duplicate_count??'—'}`;
